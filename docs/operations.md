...
By default, `kanidm-bunny` maintains a self-signed Kanidm origin certificate and key at `/data/chain.pem` and `/data/key.pem`. Public clients see Bunny's certificate for `https://idm.svee.eu`, not this origin certificate. Bunny should connect over plain HTTP to the local Caddy listener on port `8080`; Caddy uses this certificate only for the local HTTPS hop to Kanidm. Self-signed origin TLS is generated by `/usr/local/bin/kanidm-cert-helper`, a small static Go helper; the final Kanidm image does not include an OpenSSL runtime.
